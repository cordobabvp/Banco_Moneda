...
bienvenida.addEventListener('submit', (e) => {
    ...
    if (dniUsuario === '' || nombre.value === '' || apellidoUsuario === ''){
...
formulario.addEventListener('submit', (e) => {
    ...
    <p>El monto correspondiente en ARS$ es ${calculadora(selOp.value, select.value)}</p>`; //averiguar pq no define el resultado
});
...
const calculadora = (operacion, moneda) => {
    let resultado = 0
...
turno.addEventListener('click', () => {
    document.getElementById('inputDate').style.display = '';
    contTurno.innerHTML = `
    <p>Solicita un turno para atención presencial en nuestra sucursal</p>
    `;
    document.getElementById('turno').style.display = 'none';
})
...
inputDate.addEventListener('input', () => {
    contTurno.innerHTML = `
    Fecha elegida ${inputDate.value}hs
    <button id='confTurno'>Confirmar</button>`;
})
